...
app.use(bodyParser.urlencoded({limit: "50mb", extended: true, parameterLimit:50000}));app.use("/api/auth", authRouter);
app.use(express.json({limit: '50mb'}));
        ...
        switch (data.type) {
          case 'create':
            ...
            if (userWallet.wallet - data.payload.amount < 0) {
              response = { ...response, status: 400, error: "You dont have enough chips", data: null }
              return ws.send(JSON.stringify(response));
            }
            let checkChallenge = await challengesController.checkChallengeLimit(data.payload.userId)
            if (checkChallenge) {
              response = { ...response, status: 400, error: "You can Set Maximum 3 Challenges at Once", data: null }
              return ws.send(JSON.stringify(response));
            }
            let sameAmountChallenge = await challengesController.checkSameAmountChallenge({ userId: data.payload.userId, amount: data.payload.amount })
            if (sameAmountChallenge.length > 0) {
              response = { ...response, status: 400, error: "Same Amount Challenge already exist", data: null }
              return ws.send(JSON.stringify(response));
            }
            let checkPlayingOrHold = await challengesController.checkPlayingOrHold(data.payload.userId)
            if (checkPlayingOrHold.length > 0) {
              response = { ...response, status: 400, error: "Update Your Result In Previous Match First", data: null }
              return ws.send(JSON.stringify(response));
            }
            var config = {
              method: 'get',
            ...
            if (!challenge) {
              response = { ...response, status: 400, error: "challenge not created", data: null }
              return ws.send(JSON.stringify(response));
            }
            let user=await userController.existingUserById( {id: data.payload.userId})
            await sendFCM(data.payload.amount,user.username)
            await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:true})
            await challengesController.updateChallengeById({ _id: data.payload.challengeId,state: "playing" })
            // Implement your create operation here
            break;
          case 'play':
  
            let currentChallenge = await challengesController.getChallengeByChallengeId(data.payload.challengeId)
            if (currentChallenge.state == "requested") {
              response = { ...response, status: 400, error: "Request Cancelled", data: null }
              return ws.send(JSON.stringify(response));
            }
            let playerWallet = await accountController.getAccountByUserId(data.payload.userId)
            ...
            if (checkRequestedChallenges.length > 0) {
              response = { ...response, status: 400, error: "you have already requested a game", data: null }
              return ws.send(JSON.stringify(response));
            }
            ...
            currentChallenge = await challengesController.updateChallengeById(currentChallenge)
            if (!currentChallenge) {
              response = { ...response, status: 400, error: "challenge not created", data: null }
              return ws.send(JSON.stringify(response));
            }
            const reap=await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:true})
          ...
          case 'cancel':
            ...
            let canecelledChallenge = await challengesController.updateChallengeById(cancelChallenge)
            if (!canecelledChallenge) {
              response = { ...response, status: 400, error: "challenge not created", data: null }
              return ws.send(JSON.stringify(response));
            }
            await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:false})
          ...
          case 'delete':
            ...
            let deletedChallenge = await challengesController.updateChallengeById(challengeObj)
            if (!deletedChallenge) {
              response = { ...response, status: 400, error: "Challenge not found", data: null }
              return ws.send(JSON.stringify(response));
            }
            await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:false})
            break;
          case 'cancelRequestedOnPageChange':
            ...
            await challengesController.cancelRequestedChallengesByPlayerId(data.payload.userId)
            await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:false})
            break;
          case 'deleteOpenChallengesOfCreator':
            console.log('seleted too');
            await challengesController.deleteOpenChallengesCreator(data.payload.userId)
            await userController.updateUserByUserId({ _id: data.payload.userId,hasActiveChallenge:false})
            break;
          case 'startGame':
            ...
            let startGameChallenge = await challengesController.updateChallengeById({ _id: data.payload.challengeId, state: "playing" })
            ...
            response = { ...response, status: 200, error: null, data: null, challengeRedirect: true, challengeId: startGameChallenge._id }
    ...
    try {
      ...
      switch (data.type) {
        case 'getChallengeByChallengeId':
          try {
            let challenge = await challengesController.getChallengeById(data.payload.challengeId)
            if (!challenge) {
              response = { ...response, status: 400, error: "Challenge not found", data: null }
              return ws.send(JSON.stringify(response));
            }
            ...
            if (challenge.creator._id == data.payload.userId || challenge.player._id == data.payload.userId) {

              response = { ...response, status: 200, error: null, data: challenge }
              if (challenge.player._id == data.payload.userId) {
                await challengesController.updateChallengeById({ _id: challenge._id, firstTime: false })
              }
              return ws.send(JSON.stringify(response))
            }
            response = { ...response, status: 400, error: "Not Authorized", data: null }
            return ws.send(JSON.stringify(response));
          } catch (error) {
            console.log("error.message", error.message)
            response = { ...response, status: 400, error: error.message, data: null }
            return ws.send(JSON.stringify(response));
          }
      }
    } catch (error) {
  ...
  ws.on('message', async (message) => {
    try {
      ...
      switch (data.type) {
        case 'getUserWallet':
          try {
              let wallet = await accountController.getAccountByUserId(data.payload.userId)
              ws.send(JSON.stringify( { ...response, status: 200, error: null, data: wallet }));
          } catch (error) {
            response = { ...response, status: 400, error: error.message, data: null }
            return ws.send(JSON.stringify(response));
          }
        case 'updatePlayersWallet':
          try {
            let challenge = await challengesController.getChallengeById(data.payload.challengeId)
            if (!challenge) {
              response = { ...response, status: 400, error: data.payload, data: null }
              return ws.send(JSON.stringify(response));
            }
            let playerWallet = await accountController.getAccountByUserId(challenge.player._id)
            let creatorWallet = await accountController.getAccountByUserId(challenge.creator._id)
            if (challenge) {
              console.log('-------coming here ---------');
              if (connections.get(data.payload.creatorId) !== undefined) {
                let connection = connections.get(data.payload.creatorId)
                let creatorResponse = { ...response, status: 200, error: null, data: creatorWallet }
              ...
              if (connections.get(data.payload.playerId) !== undefined) {
                ...
                let playerResponse = { ...response, status: 200, error: null, data: playerWallet }
                connection.send(JSON.stringify(playerResponse))
              }
              connections.set(`guide${data.payload.playerId}`, ws);
              if (connections.get(`guide${data.payload.playerId}`) !== undefined) {
                console.log("working player guide")
                let connection = connections.get(`guide${data.payload.playerId}`)
                let playerResponse = { ...response, status: 200, error: null, data: playerWallet }
                connection.send(JSON.stringify(playerResponse))
              }
            }
