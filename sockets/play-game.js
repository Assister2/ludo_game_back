    ...
    socket.on("message", async (message) => {
        ...
        switch (data.type) {
            case "create":
                ...
                const userWallet = await accountController.getAccountByUserId(data.payload.userId);
                ...
                const checkChallenge = await challengesController.checkChallengeLimit(data.payload.userId);
                if (checkChallenge)
                ...
                const checkPlayingOrHold = await challengesController.checkPlayingOrHold(data.payload.userId);
            ...
            case "play":
                ...
                try {
                    let currentChallenge = await challengesController.getOpenChallengeByChallengeId(data.payload.challengeId);
                    if (!currentChallenge) {
                    ...
                    if (playerWallet.wallet - currentChallenge.amount < 0) {
                        return socket.send(JSON.stringify({ status: 400, error: "You don't have enough chips", data: null }));
                    }
