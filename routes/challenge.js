...
Router.get("/getChallengeByChallengeId/:challengeId", verifyToken, async (req, res) => {
    try {
        if (!req.params.hasOwnProperty("challengeId")) {
            return responseHandler(res, 400, null, "Fields are missing");
        }
        let user = req.user
        let challenge = await challengesController.getChallengeById(req.params.challengeId)
        if (!challenge) {
            return responseHandler(res, 400, null, "Challenge not found");
        }
        else {
            if ((challenge.creator._id.toString() == user.id.toString() || challenge.player._id.toString() == user.id.toString()) && challenge.state == "started") {
    ...
    try {
        ...
        if (!req.body.hasOwnProperty("image")) {
            ...
            if (challenge.results[winner !== ""]) {
                return responseHandler(res, 400, null, "You have already submitted the result");
            }
            console.log('yes1111111');
            let challengeObj = {
                ...challenge._doc,
                results: { [winner]: "win", [looser]: challenge.results[looser] },
                winnerScreenShot: { [winner]: file, [looser]: challenge.winnerScreenShot[looser] }
            }
            console.log('yes1111112');

            if (challenge.results[looser] == "" || challenge.results[looser] == "win") {
            ...
            if (challenge.results[looser] == "lost") {
                ...
                historyWinner.amount = Number((challenge.amount - (challenge.amount * 6) / 100))
                historyWinner.type = "won"
                await historyWinner.save()

                await accountController.updateAccountByUserId({ ...userWallet._doc, wallet: userWallet.wallet + amount, winningCash: userWallet.winningCash + amount, totalWin: userWallet.totalWin + challenge.amount })
                ...
                if (referUser.referer) {
                    ...
                    await accountController.increaseRefererAccount({ userId: referalAccount._id, amount: (challenge.amount) })
                }
            }
...
Router.post("/loose/:id", verifyToken, async (req, res) => {
    ...
    try {
        if (!req.params.hasOwnProperty("id")) {
            ...
            let looserWallet = await accountController.getAccountByUserId(looserUserId)
            ...
            if (challenge.results[winner] == "win") {
                ...
                let wall = { ...userWallet._doc, wallet: userWallet.wallet + amount, winningCash: userWallet.winningCash + amount, totalWin: userWallet.totalWin + challenge.amount }
...
Router.post("/cancel/:id", verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (!req.body.hasOwnProperty("cancellationReason")) {
            ...
            // let deductedAmount = amount;
            // let deduction = 0;
            let cancellerWallet = await accountController.getAccountByUserId(challenge[canceller]._id)
            let otherPlayerWallet = await accountController.getAccountByUserId(challenge[otherPlayer]._id)
            // deduction = (amount * 5) / 100;
            // if (amount >= 500) {
            //     deduction = (amount * 5) / 100;
            // }
            ...
            let challengeObj = {
                ...challenge._doc,
                results: { [canceller]: "cancelled", [otherPlayer]: challenge.results[otherPlayer] },
            ...
            if (challenge.results[otherPlayer] == "cancelled") {
                challengeObj.state = "resolved"
                cancellerWallet =  await accountController.updateAccountByUserId({ ...cancellerWallet._doc, wallet: cancellerWallet.wallet + challenge.amount, depositCash: cancellerWallet.depositCash + challenge.amount })
                await accountController.updateAccountByUserId({ ...otherPlayerWallet._doc, wallet: otherPlayerWallet.wallet + challenge.amount, depositCash: otherPlayerWallet.depositCash + challenge.amount })
            }
