...
const { responseHandler, generate, randomIntFromInterval } = require("../helpers");
...
router.post("/signup", async (req, res) => {
  try {
    if (
      ...
      if (user) {
        return responseHandler(res, 400, null, "Already registered please try to login"); 
      } 
  ...
  try {
    ...
    if (!user) {
      ...
      if ((seconds <= 3600) && (user.otp.count >= 5)) {
        return responseHandler(res, 400, null, "Can Request For 5 OTP In One hour Maximum");
      }
      user.otp = {
        code: generate(6),
        updatedAt: new Date(),
    ...
    if (!req.body.hasOwnProperty("phone") || !req.body.hasOwnProperty("otp")) {
      ...
      if (!user) {
        ...
        if (user.otp.code != req.body.otp) {
          ...
          await _app
          .messaging()
          .subscribeToTopic(token, topic).then(resp=>{
            console.log(resp);
