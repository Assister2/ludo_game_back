...
*/
    cancelRequestedChallengesByPlayerId: async (playerId) => {
        try {
            let challenge = await ChallengeModel.updateMany({ player: playerId,state:"requested" },
 ...
 */
    deleteOpenChallengesCreator: async (creatorId) => {
        try {
            let challenge = await ChallengeModel.updateMany({ creator: creatorId, state: "open" },
                { $set: { status: 0 } },
                { new: true })
            return challenge;
        } catch (error) {
 ...
 */
                deleteRequestedChallenges: async (creatorId) => {
                    try {
                        let challenge = await ChallengeModel.updateMany({ creator: creatorId, state: "requested" },
                            { $set: { status: 0 } },
                            { new: true })
                        return challenge;
                    } catch (error) {
 ...
 */
    cancelRequestedChallenges: async (creatorId) => {
        try {
            let challenge = await ChallengeModel.updateMany({ player: creatorId, state: "requested" },
                { $set: { state:"open",player:null } },
                { new: true })
            return challenge;
        } catch (error) {
...
*/
    getAllChallenges: async (challengeObject) => {
        try {
            let challenge = await ChallengeModel.find({ status: 1, state: { $ne: "resolved" } }).populate("creator player", "username profileImage")
            return challenge;
        } catch (error) {
            console.log("error", error)
            throw error
 ...
 */
    getChallengeByChallengeId: async (challengeId) => {
        try {
            let challenge = await ChallengeModel.findById(challengeId).populate("creator", "username")
            return challenge;
        } catch (error) {
            console.log("error", error)
            throw error
 ...
 */
    getChallengeById: async (challengeId) => {
        try {
            let challenge = await ChallengeModel.findById(challengeId).populate("creator player", "username")
            return challenge;
        } catch (error) {
            console.log("error", error)
            throw error
